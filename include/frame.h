...
{
public:
    Frame(std::vector<std::int32_t>& buffer, const std::size_t frame, const std::size_t channels);
    ...
    Frame& operator=(std::initializer_list<std::int32_t> values);

    /* Convenience method to assign the same value to each channel of this frame */
    Frame& operator=(const std::int32_t value);
